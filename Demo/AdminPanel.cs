...
public class AdminPanel : Panel
{
    ...
    public AdminPanel()
    {
        ...
        _newRole.Items.AddRange("user", "admin");
        _newStatus.Items.AddRange("active", "blocked");
        _editRole.Items.AddRange("user", "admin");
        _editStatus.Items.AddRange("active", "blocked");
        ...
        Add(layout, Lbl("Логин:"), 1, 2);
        Add(layout, _newLogin, 1, 3);
        Add(layout, Lbl("Пароль:"), 1, 4);
        Add(layout, _newPass, 1, 5);
        Add(layout, Lbl("ФИО:"), 1, 6);
        Add(layout, _newFio, 1, 7);
    ...
    private void OnUserSelected(object? sender, EventArgs e)
    {
        ...
        {
            MessageBox.Show("Ошибка связи с БД.", "Ошибка");
        }
    }
